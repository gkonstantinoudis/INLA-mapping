...
```{r fig.height = 6, fig.width = 5, fig.align = "center", message=F, warning=F}
...
You can play with the cutoff, max.edge and offset. What do you see? For good introduction to mesh please refer to https://folk.ntnu.no/fuglstad/Lund2016/Session6/spde-tutorial.pdf chapter 2, and also to https://haakonbakka.bitbucket.io/btopic114.html. 

## Population density

What I normally do for the population density is use the Voronoi tesselation linked with the mesh I created and calculate the number of points per polygon divided by the corresponding area.
...
To define spde we use the R-INLA function inla.spde2.pcmatern(). For the prior selection we used the PC-priors (https://arxiv.org/abs/1503.00256)
...
Now everything is ready to fit an LGCP
...
where the red areas is what the model suggested as high-risk and the yellow are the true high-risk areas. 

As the last part of this tutorial we will assess the hyperparameters (see also https://haakonbakka.bitbucket.io/btopic108.html).

```{r fig.align = "center", height = 3}
...
What do you see here? How can we interpret these plots?
